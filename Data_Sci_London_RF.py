...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 29 15:42:40 2017
...
# Data_read
 ...
 X_all=np.r_[X_train, X_test] #Merge 2 data sets
 lowest_bic = np.infty # Got a ∞
 bic = []
 n_components_range = range(1, 7)
# Obtain the optimal GMM models 
 ...
 g=best_gmm
 g.fit(X_all)
 # optimal components = 4 means 40 parameters --> 4 parameters
 # predict_proba() means to obtain the probality for each clusters
 X =g.predict_proba(X_train)
# Obtian the optimal 4 input parameters for 1000 train samples
 clf=RandomForestClassifier(n_estimators=1000, criterion='entropy', max_depth=5, min_samples_split=10,
  min_samples_leaf=3, max_features='auto', bootstrap=False, oob_score=False, n_jobs=1, random_state=33,
  verbose=0)
# Set RF models parameters
